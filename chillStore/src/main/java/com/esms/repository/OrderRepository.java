...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @Query(value = "WITH ProductRevenue AS ( " +
            ...
            "), " +
            "RankedProducts AS ( " +
            "  SELECT pr.name, pr.revenue, " +
            "    (pr.revenue * 100.0 / tr.total) AS percentage, " +
            "    ROW_NUMBER() OVER (ORDER BY pr.revenue DESC) AS rn " +
            "  FROM ProductRevenue pr CROSS JOIN TotalRevenue tr " +
            ") " +
